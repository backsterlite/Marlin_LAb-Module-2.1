...
--
-- Хост: 127.0.0.1:3306
-- Время создания: Янв 03 2020 г., 00:40
...
(8, 'blanditiis', NULL, 6),
(9, 'No-Category', NULL, 18),
(11, 'aut', NULL, 0),
(12, 'voluptates', NULL, 1),
(13, 'cupiditate', NULL, 0),
(14, 'aliquid', NULL, 0),
(15, 'numquam', NULL, 1),
(16, 'est', NULL, 0),
(17, 'doloribus', NULL, 1),
...
(3, 'backster', 'sdfzdf', 13, 38, 1, '2019-12-31 02:16:47', NULL),
(4, 'backster', 'фывывфывф', 13, 38, 1, '2019-12-31 02:18:45', NULL);
...
--
-- Дублирующая структура для представления `post_with_tags`
-- (См. Ниже фактическое представление)
--
CREATE TABLE `post_with_tags` (
`post_id` int(11)
,`title` varchar(1500)
,`slug` varchar(2000)
,`content` text
...
(13, 'coilofluck@gmail.com', '$2y$10$ZHRDt4.qRnstkq.hT2QurOyIoFJFJHOLoUBzqpfoTEsHkxOXdEDrK', 'backster', 0, 1, 1, 1, 1576781950, 1577636003, 1, '5dffd799a442f.jpg'),
(15, 'viktoria@gmail.com', '$2y$10$6TyNXN73okuYWicCOTn9WuKsrA5bzOpQm.IqNsmpAyNmWeu7LzsZi', 'vik', 0, 1, 1, 2, 1577807950, 1577807983, 0, '5e0b704e03ce4.jpg');
...
('QduM75nGblH2CDKFyk0QeukPOwuEVDAUFE54ITnHM38', 74, 1577807983, 1578347983);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `postst`  AS  select `posts`.`id` AS `id`,`posts`.`title` AS `title`,`posts`.`slug` AS `slug`,`posts`.`content` AS `content`,`posts`.`date` AS `date`,`posts`.`image` AS `image`,`posts`.`description` AS `description`,`posts`.`category_id` AS `category_id`,`posts`.`user_id` AS `user_id`,`posts`.`status` AS `status`,`posts`.`views` AS `views`,`posts`.`is_featured` AS `is_featured`,`posts`.`created_at` AS `created_at`,`posts`.`updated_at` AS `updated_at` from `posts` where `posts`.`id` = NULL order by `posts`.`id` desc ;

-- --------------------------------------------------------

--
-- Структура для представления `post_with_tags`
--
DROP TABLE IF EXISTS `post_with_tags`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `post_with_tags`  AS  select `post_tags`.`post_id` AS `post_id`,`s`.`title` AS `title`,`s`.`slug` AS `slug`,`s`.`content` AS `content`,`s`.`user_id` AS `user_id`,`s`.`date` AS `date`,`s`.`image` AS `image`,`s`.`status` AS `status`,`s`.`is_featured` AS `is_featured`,`s`.`views` AS `views`,`s`.`description` AS `description` from (`post_tags` left join `posts` `s` on(`post_tags`.`post_id` = `s`.`id`)) where `post_tags`.`tag_id` = '1' ;
